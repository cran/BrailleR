...
The UniDesc() function can deliver all of this with minimal effort from the user. In addition, the output HTML file is opened automatically, giving the blind user immediate access to the information. The content is presented using sufficiently marked up HTML code including headings and tables so that the blind user can make best use of their  screen reading software. All graphs included in the HTML file can be presented using a text description available from the VI() functionality of the BrailleR package.

The remainder of this document is a copy of the HTML file that is created when the user issues the command: 
```{r eval=FALSE}
with(airquality, UniDesc(Ozone))
```
